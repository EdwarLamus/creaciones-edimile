...
/**
 * Servicio de generación de PDF para facturas de venta.
 *
 * Paleta corporativa Creaciones Edimile (idéntica a CotizacionPdfService):
 ...
 */
@Service
public class VentaFacturaPdfService {

    private static final Logger log = LoggerFactory.getLogger(VentaFacturaPdfService.class);
    ...
    private static final DateTimeFormatter FMT_FECHA =
            DateTimeFormatter.ofPattern("dd 'de' MMMM 'de' yyyy", LOCALE_CO).withZone(ZONA);
    private static final NumberFormat FMT_MONEY = NumberFormat.getCurrencyInstance(LOCALE_CO);
    ...
    public byte[] generar(Venta venta) {
        ...
        try {
            ...
            writer.setPageEvent(new FooterEvent(venta.getNumero()));
            doc.open();

            doc.add(buildEncabezado(venta));
            doc.add(Chunk.NEWLINE);
            doc.add(buildInfoSection(venta));
            doc.add(Chunk.NEWLINE);
            doc.add(buildTablaItems(venta));
            doc.add(Chunk.NEWLINE);
            doc.add(buildTotales(venta));

            if (venta.getObservaciones() != null && !venta.getObservaciones().isBlank()) {
                doc.add(Chunk.NEWLINE);
                doc.add(buildObservaciones(venta.getObservaciones()));
            }

            doc.add(buildMensajeFinal(venta));

        } catch (DocumentException | IOException e) {
            log.error("Error al generar PDF de factura {}", venta.getNumero(), e);
    ...
    /** Encabezado negro con logo + nombre empresa + datos de la factura */
    private PdfPTable buildEncabezado(Venta venta) throws DocumentException, IOException {
        ...
        Paragraph facLabel = new Paragraph("FACTURA DE VENTA", fontTitulo());
        facLabel.setAlignment(Element.ALIGN_RIGHT);
        infoCell.addElement(facLabel);

        Paragraph numeroPar = new Paragraph(venta.getNumero(), fontSubtitulo());
        numeroPar.setAlignment(Element.ALIGN_RIGHT);
        infoCell.addElement(numeroPar);

        String fechaStr = venta.getFechaCreacion() != null
                ? FMT_FECHA.format(Instant.ofEpochSecond(venta.getFechaCreacion().getSeconds()))
        ...
        Paragraph estadoPar = new Paragraph("Estado: " + estado, fontSubtitulo());
        estadoPar.setAlignment(Element.ALIGN_RIGHT);
        infoCell.addElement(estadoPar);

        if (venta.getMetodoPago() != null) {
            Paragraph metodoPar = new Paragraph("Pago: " + venta.getMetodoPago().name(), fontSubtitulo());
            metodoPar.setAlignment(Element.ALIGN_RIGHT);
            infoCell.addElement(metodoPar);
        }

        // Si viene de cotización, mencionarlo
        if (venta.tieneCotizacion()) {
            Paragraph cotPar = new Paragraph("Ref. cotización: " + venta.getCotizacionNumero(), fontSubtitulo());
            cotPar.setAlignment(Element.ALIGN_RIGHT);
            infoCell.addElement(cotPar);
        }
    ...
    private PdfPTable buildInfoSection(Venta venta) throws DocumentException {
        ...
        table.addCell(buildInfoBox("DATOS DEL CLIENTE",
                new String[][]{
                        {"Nombre",    venta.getClienteNombre()},
                        {"Documento", venta.getClienteDocumento()},
                        {"Teléfono",  venta.getClienteTelefono()},
                        {"Email",     venta.getClienteEmail()},
                        {"Dirección", venta.getClienteDireccion()},
                }, true));

        table.addCell(buildInfoBox("DATOS DE VENTA",
                new String[][]{
                        {"Vendedor",   venta.getUsuarioNombre()},
                        {"Método pago", venta.getMetodoPago() != null ? venta.getMetodoPago().name() : "—"},
    ...
    /** Tabla de ítems */
    private PdfPTable buildTablaItems(Venta venta) throws DocumentException {
    ...
    private PdfPTable buildTotales(Venta venta) throws DocumentException {
        ...
        addTotalRow(totales, "Subtotal:", FMT_MONEY.format(venta.getSubtotal()));
        if (venta.getDescuentoPorcentaje() > 0) {
            addTotalRow(totales,
                    "Descuento (" + (int) venta.getDescuentoPorcentaje() + "%):",
                    "- " + FMT_MONEY.format(venta.getDescuentoValor()));
        }
        if (venta.getIvaPorcentaje() > 0) {
            addTotalRow(totales,
                    "IVA (" + (int) venta.getIvaPorcentaje() + "%):",
                    FMT_MONEY.format(venta.getIvaValor()));
        }

        // Fila TOTAL destacada en verde
        ...
        PdfPCell valTotal = new PdfPCell(new Phrase(FMT_MONEY.format(venta.getTotal()), fontTotal()));
    ...
    private Paragraph buildMensajeFinal(Venta venta) {
        String msg = "Gracias por su compra en Creaciones Edimile. ";
        if (venta.isPagada()) {
            msg += "Esta factura está completamente cancelada.";
        } else {
            msg += "Por favor realice su pago a la mayor brevedad posible.";
        }
        Paragraph p = new Paragraph(
                "\n" + msg,
    ...
    private static class FooterEvent extends PdfPageEventHelper {
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            cb.showText("Factura " + numero + "  |  Creaciones Edimile");
            cb.endText();
